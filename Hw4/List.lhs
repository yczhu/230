...
{-@ measure size_l :: List (List a) -> Int
size_l (Emp) = 0
size_l ((:+:) x xs)  = size x + size_l xs
@-}

{-@ invariant {v:List a | 0 <= size v} @-}
{-@ invariant {v:ListNE (List a) | 0 < size_l v} @-}
\end{code}
...
--prop_bind = lAssert (length (Emp +++ l2) == 2)
--prop_bind_2 = lAssert (length (l2 +++ Emp) == 2)
--prop_bind_3 = lAssert (length (l1 +++ l2) == 3)

{-@ concat :: xs:List (List a) -> ListN a (size_l xs) @-}
